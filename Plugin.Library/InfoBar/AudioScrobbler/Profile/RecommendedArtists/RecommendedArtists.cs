...
namespace Fuse.Plugin.Library.Info.AudioScrobbler.Profile
{
	...
	/// </summary>
	public class RecommendedArtists : Content
	{
		
		private QueryInfo last_query;
		private PagedList <RecommendedArtist> list = new PagedList <RecommendedArtist> ();
		private PageNavigator <RecommendedArtist> page_navigator;
		...
		{
			page_navigator = new PageNavigator <RecommendedArtist> (list);
		...
		public override string GetQuery (QueryInfo query)
		{
			...
			string url = "http://ws.audioscrobbler.com/1.0/user/{0}/systemrecs.xml";
		...
		public override void Load (XmlDocument doc, QueryInfo query)
		{
			...
			XmlNodeList node_list = doc.GetElementsByTagName ("recommendations");
			if (node_list.Count == 0)
				return;
			
			foreach (XmlNode node in node_list[0].ChildNodes)
				if (node.LocalName == "artist")
					list.Add (new RecommendedArtist (node));
